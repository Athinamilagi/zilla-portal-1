...
router.post('/credit-memos/list', async (req, res) => {
    try {
        const customerId = req.body.customerId.padStart(10, '0'); // Ensure 10 digits with leading zeros

        const options = {
            'method': 'POST',
            'url': 'http://AZKTLDS5CP.kcloud.com:8000/sap/bc/srt/scs/sap/zssm34_p1_credit_ws?sap-client=100',
            'headers': {
        ...
        request(options, function (error, response) {
            if (error) {
                ...
                return res.status(500).json({ error: 'Failed to fetch credit memos from SAP' });
            }
            ...
            try {
                ...
                xml2js.parseString(response.body, (parseError, result) => {
                    ...
                    try {
                        // Extract credit memos data from the parsed XML
                        const creditMemosResponse = result['soap-env:Envelope']['soap-env:Body'][0]['n0:ZSSM34_P1_CREDITResponse'][0];
                        
                        // Get the ET_INVOICE_FRONT table data
                        const invoiceData = creditMemosResponse.ET_INVOICE_FRONT ? creditMemosResponse.ET_INVOICE_FRONT[0].item : [];
                        ...
                        const formattedData = Array.isArray(invoiceData) ? invoiceData.map(item => ({
                            ...
                            description: `Credit Memo ${item.VBELN ? item.VBELN[0] : 'N/A'}`,
        ...
        console.error('Error in credit memos route:', error);
